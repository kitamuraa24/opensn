...
LBSProblem::GetOptionsBlock()
{
  ...
  params.AddOptionalParameterArray("reaction_rate_density_field_functions",
                              {},
    ...
    for (const auto& rr_info : reaction_rate_info_)
    {
      ...
      if (rr_info.total)
      {
        auto rr_ff = std::make_shared<FieldFunctionGridBased>(
          prefix + "rr_" + rr_info.reaction + "_total",
          discretization_,
      ...
      else
      {
        ...
        {
          std::ostringstream oss;
          oss << prefix << "rr_" << rr_info.reaction << "_g"
