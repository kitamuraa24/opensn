...
if __name__ == "__main__":
    ...
    xs_source.LoadFromOpenSn(os.path.join(test_dir, "rr_2d_2g_source_mat.xs"))

    xs_absorber = MultiGroupXS()
    xs_absorber.LoadFromOpenSn(os.path.join(test_dir, "rr_2d_2g_absorber_mat.xs"))
    ...
    rr_total_ff = phys.CreateReactionRateDensityFieldFunction("rr_total", "total")
    rr_abs_ff = phys.CreateReactionRateDensityFieldFunction("rr_absorption", "absorption")
    for g in range(num_groups):
        groupwise_rr_ffs.append(
            phys.CreateReactionRateDensityFieldFunction(f"rr_total_g{g}", "total", group=g)
        )
        groupwise_rr_ffs.append(
            phys.CreateReactionRateDensityFieldFunction(
                f"rr_absorption_g{g}", "absorption", group=g
            )
        )
