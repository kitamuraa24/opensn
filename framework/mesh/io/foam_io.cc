...
// skip chars until we either a ' ( ' or ' ) ' is encountered
inline bool SkipUntil(std::ifstream& in, char target)
{
  int ch;
  while ((ch = in.get()) != EOF)
    if (static_cast<char>(ch) == target) return true;
  return false;
}

// read exactly 'n' integer entries in list ( for cellZones )
inline bool ReadNInts(std::ifstream& in, int n, std::vector<int>& out)
{
                  ...
                  const std::string& fname, const char* name)
{
  ...
  // bookmark position at start of data section
  ...
  else
  {
    ...
    // rewind to the start of this line, then consume up to and including '('
    // this assumes a newline is employed '\n'
    // std::streamoff back = static_cast<std::streamoff>(line.size()) + 1; 
    in.clear();
    // in.seekg(-back, std::ios_base::cur);
    in.seekg(line_pos);

    int ch = 0;
    while ((ch = in.get()) != EOF) { if (ch == '(') break; }
    if (ch == EOF)
      throw std::logic_error(fname + ": Failed to reposition to '(' for " + std::string(name));
  }

  // read entries in data
  ...
  {
    bool ok = false;
    for (int _ = 0; _ < 32 && std::getline(in, line); ++_)
    {
      TrimInPlace(line);
...
ReadCellZones(const std::filesystem::path& path, const std::string& fname)
{
  ...
  if (!in.is_open()) return {}; // no cellZones is fine.
  SkipFoamHeader(in);

  int nzones = 0;
  std::vector<CellZoneEntry> zones;

  auto read_zone = [&](std::ifstream& in2, int /*idx*/)->bool
  {
    std::string line;
    ...
    while (std::getline(in2, line))
    {
      ...
      if (semicol != std::string::npos) line.erase(semicol);
      TrimInPlace(line);

      if (StartsWithKey(line, "cellLabels") || 
          StartsWithKey(line, "addressing") ||
          StartsWithKey(line, "labelList"))
      {
        ...
        int nlab = 0;
        {
          ...
          while (true) {
            ...
            if (!std::getline(in2, l2)) return false;
            TrimInPlace(l2);
            if (l2.empty() || StartsWithKey(l2, "//")) continue;
...
MeshIO::FromOpenFOAM(const UnpartitionedMesh::Options& options)
{
  ...
  // read " points "
  {
    ...
    auto read_pt = [&](std::ifstream& in2, int)->bool{
      ...
      verts.emplace_back(x,y,z); return true;
    };
